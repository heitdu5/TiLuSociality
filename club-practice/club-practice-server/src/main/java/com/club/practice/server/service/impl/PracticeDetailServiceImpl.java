package com.club.practice.server.service.impl;

import com.alibaba.fastjson.JSON;
import com.alibaba.nacos.common.utils.CollectionUtils;
import com.club.practice.api.enums.AnswerStatusEnum;
import com.club.practice.api.enums.CompleteStatusEnum;
import com.club.practice.api.enums.IsDeletedFlagEnum;
import com.club.practice.api.enums.SubjectInfoTypeEnum;
import com.club.practice.api.req.*;
import com.club.practice.api.vo.*;
import com.club.practice.server.dao.*;
import com.club.practice.server.entity.dto.SubjectDTO;
import com.club.practice.server.entity.dto.SubjectDetailDTO;
import com.club.practice.server.entity.dto.SubjectOptionDTO;
import com.club.practice.server.entity.dto.UserInfo;
import com.club.practice.server.entity.po.*;
import com.club.practice.server.rpc.UserRpc;
import com.club.practice.server.service.PracticeDetailService;
import com.club.practice.server.util.DateUtils;
import com.club.practice.server.util.LoginUtil;
...
public class PracticeDetailServiceImpl implements PracticeDetailService {
    ...
    public Boolean submit(SubmitPracticeDetailReq req) {
        ...
        List<PracticeSetDetailPO> minusList = practiceSetDetailPOS.stream()
                .filter(item -> !practiceDetailPOList.stream()// “！”作差集
    ...
    public Boolean submitSubject(SubmitSubjectDetailReq req) {
        ...
        if (req.getSubjectType().equals(SubjectInfoTypeEnum.JUDGE.getCode())) {
            ...
            if (correctAnswer.length() > 0) {
                correctAnswer.deleteCharAt(correctAnswer.length() - 1);//删除最后一个逗号
            }
        }
        ...
        totalMap.forEach((key, val) -> {
            ...
            if (!Objects.equals(val, 0)) {
                rate = new BigDecimal(correctCount.toString()).divide(new BigDecimal(val.toString()), 4,
                        BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(100));//除以、四位小数、四舍五入、百分率
            }
