...
public class EsRestClient {
    ...
    public static  boolean insertDoc(EsIndexInfo esIndexInfo ,EsSourceData esSourceData){
        try {
            ...
            log.error("insertDoc.exception:{}",e.getMessage());
        }
        return false;
    }
    ...
    public static boolean delete(EsIndexInfo esIndexInfo) {
        try {
            DeleteByQueryRequest deleteByQueryRequest = new
                    DeleteByQueryRequest(esIndexInfo.getIndexName());
    ...
    public static boolean deleteDoc(EsIndexInfo esIndexInfo,String docId) {
        try {
            ...
            log.info("deletedDoc.response:{}", JSON.toJSONString(response));
            return true;
        } catch (Exception e) {
     ...
     * @return
     */
    public static Map<String, Object> getDocById(EsIndexInfo esIndexInfo,String docId,String [] feilds) {
        try {
            GetRequest getRequest = new GetRequest(esIndexInfo.getIndexName());
            getRequest.id(docId);
            FetchSourceContext fetchSourceContext = new FetchSourceContext(true, feilds, null);
    ...
    public static SearchResponse searchWithTermQuery(EsIndexInfo esIndexInfo, EsSearchRequest esSearchRequest) {
        try {
            ...
            SearchRequest searchRequest = new SearchRequest(esIndexInfo.getClusterName());
            searchRequest.searchType(SearchType.DEFAULT);//这是最常用的搜索类型。它执行两个阶段的搜索：首先执行查询，然后从匹配的分片中获取数据并进行排序。高效。
