...
import com.club.subject.common.util.IdWorkerUtil;
...
public class SubjectInfoDomainServiceImpl implements SubjectInfoDomainService {
    ...
    public void add(SubjectInfoBO subjectInfoBO) {
        ...
        subjectInfoEs.setDocId(new IdWorkerUtil(1, 1, 1).nextId());//雪花算法
        subjectInfoEs.setSubjectId(subjectInfo.getId());
        subjectInfoEs.setSubjectAnswer(subjectInfoBO.getSubjectAnswer());
        subjectInfoEs.setCreateTime(new Date().getTime());//getTime()变为时间戳，适应实体字段是Long类型
    ...
    private void assembleSubjectCursor(SubjectInfoBO subjectInfoBO, SubjectInfoBO bo) {
        ...
        //因为调用者是查询题目接口，不是都用在练题时候，所以有时候没有labelId和categoryId
    ...
    public List<SubjectInfoBO> getContributeList() {
        ...
        typedTuples.forEach( (rank ->{
            ...
            UserInfo userInfo = userRpc.userInfo(rank.getValue());
